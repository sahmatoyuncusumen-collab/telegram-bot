...
# --- TƏHLÜKƏSİZLİK YOXLAMASI (ƏLAVƏ EDİLMƏYİ UNUDULAN FUNKSİYA) ---
def run_pre_flight_checks():
    """Skriptin yalnız Render serverində işlədiyindən əmin olur."""
    if not DATABASE_URL or not TOKEN:
        print("--- XƏTA ---")
        print("DATABASE_URL və ya TELEGRAM_TOKEN tapılmadı.")
        print("Bu skript yalnız düzgün konfiqurasiya edilmiş Render serverində işləməlidir.")
        print("Lütfən, bu pəncərəni bağlayın və botu lokal olaraq işə salmayın.")
        sys.exit(1) # Skripti xəta ilə dayandırır
...
STORY_DATA = {
    'start': {
        ...
        'choices': [{'text': "🌳 Sol cığırla gedək", 'goto': 'forest_path'}, {'text': "🦇 Mağaraya daxil olaq", 'goto': 'cave_entrance'}]
    },
    'forest_path': {
        ...
        'choices': [{'text': "🔑 Qədim açarı istifadə edək", 'goto': 'open_door', 'requires_item': 'qədim açar'}, {'text': " geri dönək", 'goto': 'go_back'}]
    },
    'cave_entrance': {
        'text': "Qaranlıq mağaraya daxil olursunuz. Divardan asılmış köhnə bir açar gözünüzə dəyir. Onu götürürsünüz.",
        'get_item': 'qədim açar',
        'choices': [{'text': "Açarla birlikdə geri dönək", 'goto': 'get_key'}]
    },
    'get_key': {
        'text': "Artıq qrupun inventarında köhnə, paslı bir açar var. Bu, bəzi qapıları aça bilər. İndi hara gedirsiniz?",
        'choices': [{'text': "🌳 Meşədəki qapını yoxlayaq", 'goto': 'forest_path'}, {'text': "🧭 Məbədin girişinə qayıdaq", 'goto': 'start'}]
    },
    'open_door': {
        ...
        'choices': [{'text': "⚔️ Qılıncı götürək!", 'goto': 'treasure_found'}]
    },
    'treasure_found': {
    ...
    'go_back': {
        ...
        'choices': [{'text': "🦇 Mağaraya daxil olaq", 'goto': 'cave_entrance'}]
    }
}
...
async def show_rpg_node(context: ContextTypes.DEFAULT_TYPE, chat_id: int, node_key: str):
    ...
    if not choices:
        ...
        context.chat_data.pop('rpg_inventory', None); context.chat_data.pop('active_poll', None); return
    poll_options, choices_map = [], {}
    for choice in choices:
        if 'requires_item' in choice:
            if choice['requires_item'] in inventory:
                poll_options.append(choice['text']); choices_map[choice['text']] = choice['goto']
        else:
            poll_options.append(choice['text']); choices_map[choice['text']] = choice['goto']
    if not poll_options:
        await context.bot.send_message(chat_id, "Görünür, davam etmək üçün doğru əşyanız yoxdur. Məğlub oldunuz. 😔")
        context.chat_data.pop('rpg_inventory', None); return
    poll_message = await context.bot.send_poll(chat_id=chat_id, question=text, options=poll_options, is_anonymous=False, allows_multiple_answers=False, open_period=60)
    context.chat_data['active_poll'] = {'poll_id': poll_message.poll.id, 'chat_id': chat_id, 'choices_map': choices_map}
async def receive_poll_update(update: Update, context: ContextTypes.DEFAULT_TYPE):
    poll_data = context.chat_data.get('active_poll')
    if not poll_data or update.poll.id != poll_data['poll_id']: return
    if update.poll.is_closed:
        winning_option, max_votes = None, -1
        for option in update.poll.options:
            if option.voter_count > max_votes:
                max_votes, winning_option = option.voter_count, option.text
        context.chat_data.pop('active_poll', None)
        if winning_option:
