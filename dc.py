...
# DÜZƏLİŞ EDİLMİŞ start_command FUNKSİYASI
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("start_info_"):
        command_map = {'oyun': '/oyun','tapmaca': '/tapmaca','viktorina': '/viktorina','macera': '/macera','reyting': '/reyting gunluk','qaydalar': '/qaydalar'}
        command_name = data.split('_')[-1]
        if command_name == 'qaydalar':
            await qaydalar_command(query.message, context)
            await query.answer()
        else:
