...
# ... (Köhnə kod olduğu kimi qalır, aşağıdakı tam kodda mövcuddur)
...
async def tapmaca_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # YENİ: Cavabı keçmək üçün düymə əlavə edirik
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # YENİ: Tapmacanı keçmək üçün məntiq
    if data == "skip_riddle":
        if not context.chat_data.get('riddle_active'):
            await query.answer("Bu tapmaca artıq bitib.", show_alert=True)
            return
    ...
    if data == "register_join":
        # ... (qeydiyyat məntiqi olduğu kimi qalır)
        pass 
    elif data.startswith("game_"):
        # ... (oyun məntiqi olduğu kimi qalır)
        pass
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # YENİ: Tapmaca cavabını yoxlama məntiqi
    if context.chat_data.get('riddle_active'):
        correct_answers = context.chat_data.get('riddle_answer', [])
        if text and text.strip().lower() in correct_answers:
            # Tapmaca tapıldıqda, düyməni silmək üçün orijinal mesajı redaktə edirik
            original_message = update.message.reply_to_message
            if original_message and context.bot.id == original_message.from_user.id:
                 try:
                    await original_message.edit_reply_markup(reply_markup=None)
                 except Exception as e:
                    logger.info(f"Köhnə tapmaca mesajını redaktə edə bilmədi: {e}")

            await update.message.reply_text(f"Əhsən, [{user.first_name}](tg://user?id={user.id})! 🥳 Düzgün cavab tapıldı! ✅", parse_mode='Markdown')
            del context.chat_data['riddle_active']
            del context.chat_data['riddle_answer']
            
    # Mesaj sayma məntiqi olduğu kimi qalır
    # ...

def main() -> None:
    # ...
    application.add_handler(CommandHandler("tapmaca", tapmaca_command, filters=group_filter)) # Bu sətir artıq var
    # ...

# --- Bütün Dəyişikliklərlə Birlikdə Tam Kod (BUNU KOPYALAYIN) ---
# (Kodun tam versiyası yuxarıdakı izahlarla eynidir, sadəcə bütün funksiyalarla birlikdə təqdim olunur)
# ... (Kopyalamaq üçün tam kod)
# BU HİSSƏYƏ TAM KODU YAPIŞDIRMAQ LAZIMDIR. 
# Mən yuxarıda ayrı-ayrı göstərdiyim hissələri sonuncu tam kodun üzərində birləşdirib aşağıya yapışdırıram.
...
def run_pre_flight_checks():
    if not DATABASE_URL or not TOKEN:
        print("--- XƏTA ---"); print("DATABASE_URL və ya TELEGRAM_TOKEN tapılmadı."); print("Bu skript yalnız düzgün konfiqurasiya edilmiş Render serverində işləməlidir."); print("Lütfən, bu pəncərəni bağlayın və botu lokal olaraq işə salmayın."); sys.exit(1)
    print("Bütün konfiqurasiya dəyişənləri mövcuddur. Bot başladılır...")
