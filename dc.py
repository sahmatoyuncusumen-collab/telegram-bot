...
QUIZ_QUESTIONS = [{'question': 'Azərbaycanın paytaxtı haradır?', 'options': ['Gəncə', 'Sumqayıt', 'Bakı', 'Naxçıvan'], 'correct': 'Bakı'}] # Qısa olsun deyə kəsilib
RIDDLES = [{'riddle': 'Ağzı var, dili yox, danışdıqca cana gəlir. Bu nədir?', 'answers': ['kitab']}] # Qısa olsun deyə kəsilib
NORMAL_TRUTH_QUESTIONS = ["Uşaqlıqda ən böyük qorxun nə idi?"] # Qısa olsun deyə kəsilib
NORMAL_DARE_TASKS = ["Profil şəklini 1 saatlıq qrupdakı ən son göndərilən şəkil ilə dəyişdir."] # Qısa olsun deyə kəsilib
...
# START MENYUSU ÜÇÜN KÖMƏKÇİ FUNKSİYA
async def start_command_menu(update: Update, context: ContextTypes.DEFAULT_TYPE, is_edit: bool = False):
    ...
    keyboard = [
        ...
        [InlineKeyboardButton("🗺️ Macəra Oyunu", callback_data="start_info_macera")],
    ...
    message_to_act_on = update.message if not is_edit else update.callback_query.message
    if is_edit:
        await message_to_act_on.edit_text(text=start_text, reply_markup=reply_markup)
    else:
        await message_to_act_on.reply_text(text=start_text, reply_markup=reply_markup)
...
#... (qalan bütün oyun əmrləri olduğu kimi qalır)
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("start_info_"):
        command_map = {'oyun': '/oyun', 'tapmaca': '/tapmaca', 'viktorina': '/viktorina', 'macera': '/macera', 'reyting': '/reyting gunluk'}
